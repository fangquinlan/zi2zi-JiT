#!/usr/bin/env python3
"""Launch JiT-L/16 full-model continued pretraining from a pretrained checkpoint."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Start JiT-L/16 full-model continued pretraining on a prepared font dataset.",
    )
    parser.add_argument("--dataset-dir", default="data/sourcehan_font_training_dataset")
    parser.add_argument("--output-dir", default="run/continue_pretrain_large_sourcehan")
    ...
    parser.add_argument("--finite", dest="infinite", action="store_false")
    parser.set_defaults(infinite=True)
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--lr", type=float, default=None)
    parser.add_argument("--blr", type=float, default=8e-5)
    parser.add_argument("--warmup-epochs", type=int, default=2)
    parser.add_argument("--weight-decay", type=float, default=0.0)
    parser.add_argument("--save-last-freq", type=int, default=1)
    parser.add_argument("--eval-freq", type=int, default=1)
    parser.add_argument("--gen-bsz", type=int, default=16)
    ...
    parser.add_argument("--num-style-refs", type=int, default=1)
    parser.add_argument("--style-ref-mode", choices=["single", "mean", "max"], default="single")
...
def load_dataset_info(dataset_dir: Path) -> dict:
    ...
    if not dataset_info_path.is_file():
        raise FileNotFoundError(
            f"dataset_info.json not found under {dataset_dir}. Run prepare_sourcehan_font_training_dataset.py first."
        )
...
def maybe_download_checkpoint(
    ...
    *,
    ...
    if base_checkpoint.is_file() or dry_run:
        return
...
def main() -> None:
    ...
    train_script = repo_root / "main_jit.py"
    ...
    cmd = [
        ...
        "--num_style_refs", str(args.num_style_refs),
        "--style_ref_mode", args.style_ref_mode,
        "--img_size", "256",
    ...
    print(f"  infinite    = {args.infinite}")
    print(f"  batch_size  = {args.batch_size}")
    print(f"  lr          = {args.lr if args.lr is not None else 'auto from blr'}")
    print(f"  blr         = {args.blr}")
    print(f"  num_style_refs = {args.num_style_refs}")
    print(f"  style_ref_mode = {args.style_ref_mode}")
    print(f"  save_last_freq = {args.save_last_freq}")
